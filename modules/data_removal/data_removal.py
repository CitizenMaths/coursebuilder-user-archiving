...
#list of strings that contain admin emails for the course that should not be removed or archived at all
CM_ADMIN_EMAILS = ['<YOUR_ADMIN_EMAILS_HERE>', '<YOUR_ADMIN_EMAILS_HERE>']
...
#apf: added new class for deletion and archiving of users registered/last active over 2 years ago
class ArchivingUserDataCronHandler(utils.AbstractAllCoursesCronHandler):

    URL = '/cron/user_data/archive'
    ...
    def cron_action(self, app_context, global_state):
        #only want this to run for the course and a particular namespace
        if app_context.get_namespace_name() == '<YOUR_COURSE_NAMESPACE_HERE>':
            #set up date criteria that is two years earlier than the current datetime
            date_criteria = datetime.now()
            try:
                date_criteria = date_criteria.replace(year=date_criteria.year - 2)
            except ValueError:
                #must be 2/29
                assert date_criteria.month == 2 and date_criteria.day == 29
                date_criteria = date_criteria.replace(month=2, day=28, year=date_criteria.year-2)

            users = models.Student.all()
            anonymise_count = 0
            for user in users:
                if user is not None:
                    #check whether user meets the criteria to anonymise and archive or not
                    anonymise = False
                    if user.last_seen_on is not None and user.last_seen_on <= date_criteria:
                        anonymise = True
                    elif user.last_seen_on is None and user.enrolled_on is not None and user.enrolled_on <= date_criteria:
                        anonymise = True

                    #if anonymise is true, then check the email is not one of our admin users, if it is, set anonymise to false as we do not want to delete these users
                    if anonymise and user.email is not None and user.email in CM_ADMIN_EMAILS:
                        logging.warn('resetting anonymise to false for user who is an admin user: {ue}'.format(ue=user.email))
                        anonymise = False
                    ...
                    if anonymise:
                        ...
                        log_lastseenon = 'user last seen on or enrolled on more than two years ago: {uid}'.format(uid=user_id)
                        logging.info(log_lastseenon)

                        af_em = None
                        if af is not None:
                            #anonymise the personal data from the additional fields by replacing there values with an empty string
                            af_gn = self.replace_value_in_additional_fields(af, 'GivenName', '')
                            af_fn = self.replace_value_in_additional_fields(af_gn, 'FamilyName', '')
                            af_ea = self.replace_value_in_additional_fields(af_fn, 'EmailAddress', '')
                            af_em = self.replace_value_in_additional_fields(af_ea, 'ConfirmEmail', '')

                            #anonymised additional fields logged
                            logging.info(af_em)
                        else:
                            af_em = af

                        #create a user archive object and insert into datastore, then delete the current user in the loop
                        ua = models.UserArchive._add_new(user_id, af_em, user.enrolled_on, user.is_enrolled, user.last_seen_on)
                        ua.put()

                        #delete the user from the Student table
                        user.delete()

            logging.info('total anonymised and deleted:')
            logging.info(anonymise_count)
