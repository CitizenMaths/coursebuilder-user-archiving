...
class UserArchive(BaseEntity):

    enrolled_on = db.DateTimeProperty(indexed=True)
    ...
    def __init__(self, *args, **kwargs):
        super(UserArchive, self).__init__(*args, **kwargs)
    ...
    @classmethod
    def _add_new(cls, user_id, additional_fields, enrolled_on=None, is_enrolled=True, last_seen_on=None):
        return UserArchive(key_name=user_id, user_id=user_id, additional_fields=additional_fields, enrolled_on=enrolled_on, is_enrolled=is_enrolled, last_seen_on=last_seen_on)

    @classmethod
    def get_by_user_id(cls, user_id):
        """Load UserArchive record by user_id. Fail if user_id is not unique."""
        # Check UserArchive entities for user_id: do a double look up here:
        # first by the key_name value and then by the user_id field value.
        ua = UserArchive.get_by_key_name(user_id)
        if ua:
            return ua

        uas = UserArchive.all().filter(
            UserArchive.user_id.name, user_id).fetch(limit=2)
        if len(uas) > 1:
            raise Exception(
                'There is more than one user archive record with user_id "%s"' % user_id)

        return uas[0] if uas else None
